...
const hexToRGB = (hex: string): { r: number; g: number; b: number } => {
    ...
    const parsed = Number.parseInt(cleaned, 16) || 0xffffff;
    return {
        r: ((parsed >> 16) & 255) / 255,
        g: ((parsed >> 8) & 255) / 255,
        b: (parsed & 255) / 255,
...
export default function LaserFlow({
    ...
    useEffect(() => {
        ...
        const setSizeNow = () => {
            ...
            const sizeChanged = Math.abs(width - last.width) > 0.5 || Math.abs(height - last.height) > 0.5;
            const dprChanged = Math.abs(pixelRatio - last.dpr) > 0.01;
        ...
        const adjustDprIfNeeded = (now: number) => {
            ...
            const avgFps = samples.reduce((sum, value) => sum + value, 0) / samples.length;
            let next = currentDprRef.current;
            const base = baseDprRef.current;

            if (avgFps < lowerThreshold) {
                next = clamp(currentDprRef.current * 0.85, dprFloor, base);
            } else if (avgFps > upperThreshold && currentDprRef.current < base) {
